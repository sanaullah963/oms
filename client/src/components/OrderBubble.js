    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
            <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm transform transition-all duration-300 scale-100">
                <h3 className={`text-lg font-bold mb-3 ${isConfirm || message.startsWith('ত্রুটি') ? 'text-red-600' : 'text-green-600'}`}>
                    {title}
                </h3>
                ...
                <div className="flex justify-end space-x-3">
                    ...
                    <button
                        onClick={onConfirm}
                        className={`px-4 py-2 rounded-lg text-white font-semibold transition ${isConfirm 
...
export default function OrderBubble({ order, onUpdate }) {
    ...
    const handleStatusUpdate = async (shortcut) => {
        ...
        try {
            // API কল: PATCH /api/orders/:orderId/status
            const response = await axios.patch(`${API_BASE}/${order._id}/status`, {
                newStatus: key,
                activityNote: note 
            });
            ...
            showMessage('alert', `অর্ডার স্ট্যাটাস সফলভাবে আপডেট হয়েছে: ${key}।`, null);

        } catch (error) {
            console.error("Failed to update status:", error.response?.data || error.message);
            const errorMessage = error.response?.data?.message || 'সার্ভার এরর।';
            showMessage('alert', `ত্রুটি: স্ট্যাটাস আপডেট ব্যর্থ হয়েছে। ${errorMessage}`, null);
        } finally {
            setLoading(false);
        }
    };
    ...
    const executeDelete = async () => {
        ...
        try {
            ...
            showMessage('alert', 'সফলভাবে ডিলিট করা হয়েছে: অর্ডারটি লিস্ট থেকে সরানো হলো।', null);
    ...
    const sortedActivities = [...(order.activities || [])].sort((a, b) => 
    ...
    const statusColor = order.orderStatus === 'Pending' ? 'text-yellow-600 bg-yellow-100' : 'text-green-600 bg-green-100';
    ...
    return (
        <>
            <div className="bg-white rounded-xl shadow-lg p-4 mb-4 border border-gray-200 hover:shadow-xl transition-all duration-300">
                ...
                {/* --- মূল অর্ডার বাবল (Header) - ক্লিক করলে প্রসারিত হবে --- */}
                <div 
                ...
                <div className="flex justify-between mt-4 pt-3 border-t border-gray-100">
                    ...
                    <div className="flex space-x-2">
                        <button 
                            ...
                            onClick={() => handleCopy(order.castomerPhone, 'ফোন নম্বর কপি করা হয়েছে!')}
                            title="ফোন নম্বর কপি"
                            disabled={loading}
                        >
                        ...
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="feather feather-phone"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6.7-6.7A19.79 19.79 0 0 1 2 4.18 2 2 0 0 1 3.16 2h3a2 2 0 0 1 2 1.72v3.25a2 2 0 0 1-1.25 1.83 1.5 1.5 0 0 0-.25.13 10.9 10.9 0 0 0 5.43 5.43 1.5 1.5 0 0 0 .13-.25 2 2 0 0 1 1.83-1.25h3.25A2 2 0 0 1 22 16.92z"></path></svg>
                        </a>
                        <button 
                            className="p-2 text-sm rounded-full bg-gray-100 text-gray-600 hover:bg-gray-200 transition duration-150 shadow-md"
                            onClick={() => handleCopy(orderText, 'সম্পূর্ণ অর্ডার টেক্সট কপি করা হয়েছে!')}
                ...
                >
                    <div className="mt-4 pt-4 border-t border-gray-300">
                        ...
                        <h4 className="text-xs font-semibold mb-3 text-gray-700 uppercase tracking-wider">দ্রুত স্ট্যাটাস আপডেট:</h4>
                        <div className="flex flex-wrap gap-2 mb-6">
                            {STATUS_SHORTCUTS.map(shortcut => (
                                <button
                                    key={shortcut.key}
                                    onClick={() => handleStatusUpdate(shortcut)}
                                    className={`text-white text-xs font-medium py-1.5 px-3 rounded-full shadow-md transition duration-200 ${shortcut.color} ${loading ? 'opacity-50 cursor-not-allowed' : 'hover:ring-2 ring-offset-1 ring-opacity-50'}`}
                                    disabled={loading}
                                >
                                    {shortcut.label}
                                </button>
                        ...
                        <h4 className="text-xs font-semibold mb-3 text-gray-700 uppercase tracking-wider">অ্যাক্টিভিটি টাইমলাইন:</h4>
                        <div className="space-y-4">
                            {sortedActivities.map((activity, index) => (
                                <div key={index} className="flex items-start text-xs">
                                    <span className={`w-1/4 flex-shrink-0 font-bold ${ACTIVITY_STATUS_COLORS[activity.type] || 'text-gray-500'}`}>
                                    ...
                                    <div className="w-3/4 pl-3 border-l-2 border-dashed border-gray-200">
                                        ...
                                        <p className="text-gray-600 mt-0.5">
                                            {activity.details?.description || activity.description || activity.note || 'নোট নেই'}
                                        </p>
                                    </div>
                                </div>
                            ))}
                            {sortedActivities.length === 0 && (
                                <p className="text-xs text-gray-500 italic">এই অর্ডারের জন্য কোনো অ্যাক্টিভিটি রেকর্ড করা হয়নি।</p>
                            )}
                        </div>
                    </div>
                </div>
